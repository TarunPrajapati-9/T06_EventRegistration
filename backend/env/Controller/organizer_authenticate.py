...
def get_authenticate_organizer(f):
    @wraps(f)
    async def wrapper(request: Request, *args, **kwargs):
        ...
        try:
            ...
            organizer_details = await database['Organizer'].find_one({"_id": ObjectId(_id)})

            if organizer_details and  ObjectId(_id)== organizer_details['_id']:
